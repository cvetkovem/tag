...
void GpioInit(Gpio_t *obj, PinModes mode,  PinConfigs config, PinTypes type, uint8_t value)
{
    ...
    /* bit banding address 0x42000000+(32*0x2381C)+(bit_number*4)=0x42470380+(bit_number*4) */
    *((uint32_t *)(0x42470380 + ((uint32_t)((obj->portIndex) << 2)))) = 0x01;

    /* bit banding address 0x42400000+(32*(0x400*obj->portIndex))+(32*XXXX)+(bit_number*4) */
    /* MODER offset 0x00 */
...
void GpioDeInit(Gpio_t *obj)
{
    ...
    *((uint32_t *)(gpio_reg + 4)) = 0x00;
	...
	{
		/* use bit banding */
		*((uint32_t *)(0x42470380 + ((uint32_t)((obj->portIndex) << 2)))) = 0x00;
	}
}
...
void GpioSetInterrupt(Gpio_t *obj, IrqModes irqMode, IrqPriorities irqPriority, GpioIrqHandler *irqHandler)
{
    ...
    /* bit banding address 0x42000000+(32*0x23820)+(bit_number*4)=42470400 */
    *((uint32_t *)(0x42470400)) = 0x01;

    /* Connect EXTI Line to GPIO pin */
    /* SYSCFG_EXTILineConfig */
    /* SYSCFG->EXTICR[0]-[3] */
    /* bit banding address 0x42000000+(32*0x10008)+(32*0x04*EXTICR_NUMBER)+(bit_number*4)=42200100+(0x80*EXTICR_NUMBER)+(bit_number*4) */
    //gpio_reg = 42200100+(0x80*((uint32_t)(obj->pinIndex >> 2))) + 4*( obj->pinIndex - (obj->pinIndex >> 2)*4 )*4;
    // *((uint32_t *)(gpio_reg))      = (obj->portIndex) & 0x01;
    // *((uint32_t *)(gpio_reg + 4))  = ((obj->portIndex) >> 1) & 0x01;
    // *((uint32_t *)(gpio_reg + 8))  = ((uint32_t)(obj->portIndex) >> 2) & 0x01;
    // *((uint32_t *)(gpio_reg + 12)) = ((uint32_t)(obj->portIndex) >> 3) & 0x01;

    SYSCFG->EXTICR[(obj->pinIndex >> 2)] |= (obj->portIndex) << 4*( obj->pinIndex - (obj->pinIndex >> 2)*4 );

    /* Configure EXTI line */
    /* EXTI->IMR */
    /* bit banding address 0x42000000+(32*0x10400)+(bit_number*4)= 0x42208000 + (bit_number*4)*/
    *((uint32_t *)(0x42208000 + 4*(obj->pinIndex))) = 0x01;
    ...
    /* bit banding address 0x42000000+(32*0x10408)+(32*0x04)+(bit_number*4)= 0x42208100+(32*0x04)+(bit_number*4) */
    /* EXTI->RTSR */
    *((uint32_t *)(0x42208100 + 4*(obj->pinIndex))) = irqMode & 0x01;
    /* EXTI->FTSR */
...
void GpioRemoveInterrupt(Gpio_t *obj)
{
    ...
    /* EXTI->IMR */
    /* bit banding address 0x42000000+(32*0x10400)+(bit_number*4)= 0x42208000 + (bit_number*4)*/
    *((uint32_t *)(0x42208000 + 4*(obj->pinIndex))) = 0x00;
    ...
    /* bit banding address 0x42000000+(32*0x10408)+(32*0x04)+(bit_number*4)= 0x42208100+(32*0x04)+(bit_number*4) */
    /* EXTI->RTSR */
    *((uint32_t *)(0x42208100 + 4*(obj->pinIndex))) = 0x00;
    /* EXTI->FTSR */
    *((uint32_t *)(0x42208100 + (32*0x04) + 4*(obj->pinIndex))) = 0x00;
